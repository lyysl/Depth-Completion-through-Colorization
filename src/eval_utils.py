...
def evaluate(output_depths,
    ...
    for idx in range(n_sample):
        ...
        if normalise == 1:
            ...
            max_pixel_value = max([np.amax(output_depth), np.amax(ground_truth)])
            output_depth = output_depth/max_pixel_value
            ground_truth = ground_truth/max_pixel_value
            # Compute for normalised evaluation metrics
            mae[idx] = mean_abs_err(output_depth, ground_truth)
            rmse[idx] = root_mean_sq_err(output_depth, ground_truth)
            imae[idx] = inv_mean_abs_err(output_depth, ground_truth)
            irmse[idx] = inv_root_mean_sq_err(output_depth, ground_truth)
        else:
